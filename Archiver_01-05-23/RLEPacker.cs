...
namespace Archiver_01_05_23
{
    class RLEPacker
    {
        public delegate void ProgressBarRLEPacker(int progress);
        ...
        public int CountSimilarSymbols { get; set; } = 1;
        ...
        internal void StreamPack(Stream source, Stream dest)
        {
            ...
            DataLength = (int)source.Seek(0, SeekOrigin.End); // количество символов в файле
            ...
            Progress = (int)source.Position * 100 / DataLength;
            pbNotifyRLEPacker?.Invoke(Progress);
            while (source.Position != source.Length)
            {
                ...
                if (FirstSymbol != SecondSymbol)
                {
                    ...
                    dest.WriteByte(32); // null по кодировке ASCII
                    dest.Write(bufArrCount, 0, bufArrCount.Length);
                    dest.WriteByte(32); // null по кодировке ASCII
        ...
        internal void StreamUnpack(Stream source, Stream dest)
        {
            Progress = 0;
            pbNotifyRLEPacker?.Invoke(Progress);
            DataLength = (int)source.Seek(0, SeekOrigin.End);
            ...
            while (source.Position != source.Length)
            {
                if (FirstSymbol != 32)
                {
                    ...
                    iterator = 0;
                    while (FirstSymbol != 32 & FirstSymbol != -1)
                    {
                    ...
                    Progress = (int)source.Position / DataLength * 100;
                    pbNotifyRLEPacker?.Invoke(Progress);
                }
